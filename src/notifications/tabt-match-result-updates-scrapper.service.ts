...
import { MatchResultUpdate } from '../common/event-bus/models/event.model';
...
export class TabtMatchResultUpdatesScrapperService {
	...
	async getLatestMatchResultUpdates(): Promise<MatchResultUpdate[]> {
		const page: string = await firstValueFrom(this.httpService
		...
		const dates = Array.from(page.matchAll(regexDate),
			(item) => {
				const year = (new Date()).getFullYear();
				return new Date(
					year, Number(item[2]), Number(item[1]), Number(item[3]), Number(item[4]),
				);
